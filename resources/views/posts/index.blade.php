...
<div class="flex justify-end max-w-xl mx-auto mt-9" >
    <a class="dark:bg-fuchsia-700 dark:hover:bg-fuchsia-600 font-bold rounded-md p-3 dark:text-white shadow-md transition-colors" href="{{ route('posts.create') }}">New Post</a>
</div>
@endrole
<div id="post-data">
    ...
    <article class='post-card flex max-w-xl my-10 bg-white dark:bg-gray-600 shadow-md rounded-lg overflow-hidden mx-auto'>
        <div class='flex items-center w-full z-0 relative'>
            ...
            <div class="h-full w-full absolute opacity-20" style="z-index: -1">
                <img class="lazy object-cover object-top h-full w-full" data-original='{{ url($post->post_images()->first()->post_image_path) }}'>
            </div>
            @endif
            <div class='w-full'>
                <div class="flex flex-row justify-between mt-2 px-2 py-3 mx-3">
                    <div class="flex flex-row" >
                        ...
                        <div class="flex flex-col mb-2 ml-4 mt-1">
                            <div class='text-gray-600 dark:text-gray-100 text-sm font-semibold'>{{ App\Models\User::where('id', '=', $post->user_id)->first()->name }}</div>
                            <div class='flex w-full mt-1 flex-col'>
                                <div class='text-gray-400 dark:text-gray-300 font-thin text-xs'>
                                    ...
                                    ?>
                                    • {{ $postCreatedAt->format('D \a\t\ g:i:a - j/n/Y') }}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class='text-gray-400 font-thin text-sm mb-6 mx-3 px-2'>
                    ...
                    ?>
                    last update • {{ $postUpdatedAt->format('D \a\t\ g:i:a - j/n/Y') }}
                </div>
                <div class="flex mt-2 px-2 py-3 mx-2">
                    <a class="dark:bg-fuchsia-500 dark:text-white pl-12 pr-12 text-center rounded-md p-1 pr-2 pl-2 mb-2 font-bold shadow-md dark:shadow-gray-500/40 dark:hover:bg-fuchsia-400 transition-colors" href="{{ route('posts.single', $post->slug) }}" >Read</a>
                </div>
            </div>
        </div>
    </article>
