...
<div class="flex mx-auto max-w-screen-xl mb-10 mt-10">
    <article class="flex flex-col overflow-hidden p-4 pr-8 max-w-screen-md xl:max-w-screen-lg grow" >
        ...
        <div class="w-full flex flex-row space-x-12 justify-between items-center" >
            <a class="hover:text-violet-400 text-violet-500 dark:bg-fuchsia-700 dark:hover:bg-fuchsia-600 p-3 dark:text-zinc-300 rounded-lg transition-colors dark:shadow-md cursor-pointer" href="<?php echo e(route('posts.edit', ['post' => $post->slug])); ?>" >Edit Article</a>
        </div>
        <?php endif; ?>
        <?php if($post->post_images()->first() != null): ?>
        <div class="flex items-center w-full max-w-screen-md mb-4 mt-8 overflow-hidden h-96">
            <img class="lazy object-cover object-top h-full w-full" data-original="<?php echo e(url($post->post_images()->first()->post_image_path)); ?>" alt="">
        </div>
        <?php endif; ?>
        <h1 class="font-bold text-zinc-500 dark:text-white text-7xl text-ellipsis overflow-hidden" ><?php echo e($post->title); ?></h1>
    ...
    <div class="h-full w-auto hidden lg:flex mt-12" >
        <?php if(App\Models\User::where('id', '=', $post->user_id)->first() != null): ?>
        ...
        <div class=" w-full sm:flex sm:flex-col sm:justify-around sm:p-2 sm:pt-4 sm:pb-4 sm:dark:bg-gray-800/75 bg-white sm:shadow-md sm:rounded-lg space-y-6 w-96" >
            <div class="flex flex-row justify-around">
                <h3 class="dark:text-zinc-300 font-bold text-lg p-2 text-gray-600" ><?php echo e(App\Models\User::where('id', '=', $post->user_id)->first()->name); ?></h3>
            ...
            <div class="flex w-full justify-center" >
                <?php if(auth()->check() && auth()->user()->id != App\Models\User::where('id', '=', $post->user_id)->first()->id): ?>
                <button id="follow" type="button" class="text-gray-600 font-semibold bg-stone-300/75 hover:bg-stone-200 dark:text-white p-2 rounded-md transition-colors w-full text-center cursor-pointer <?php echo e(auth()->user()->isFollowing(App\Models\User::where('id', '=', $post->user_id)->first()) ? 'dark:bg-gray-700 dark:hover:bg-gray-600' : 'dark:bg-fuchsia-500 dark:hover:bg-fuchsia-400 '); ?>" data-user="<?php echo e(auth()->check() ? $post->user_id : ''); ?>" data-unfollow="<?php echo e(auth()->user()->isFollowing(App\Models\User::where('id', '=', $post->user_id)->first()) ? 'unfollow' : ''); ?>"><?php echo e(auth()->check() && auth()->user()->isFollowing(App\Models\User::where('id', '=', $post->user_id)->first()) ? "Unfollow" : 'Follow'); ?></button>
                <?php else: ?>
            ...
            <div class="flex flex-col items-start w-full" >
                ...
                ?>
                <span class="dark:text-gray-400 text-gray-500" ><?php echo e($userJoinDate->format('M jS Y')); ?></span>
            </div>
        </div>
        <?php endif; ?>
    </div>
...
<?php echo $__env->make('layouts.app-master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/openduct/public_html/resources/views/posts/single.blade.php ENDPATH**/ ?>